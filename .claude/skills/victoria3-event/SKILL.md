...
You are editing Paradox script for Victoria 3 version 1.12. The 1.13 update ships April 28, 2026 and adds National Cast enhancements; prefer 1.12-safe patterns and plan forward-compatibility into variable names and data shapes.
...
4. **Overriding vanilla on_actions.** Writing `on_law_enacted = { effect = { ... } }` at the top level *replaces* the vanilla on_action and breaks base-game events. Always subscribe by nesting inside `on_actions = { ... }` (pattern in reference.md §on_actions).
...
1. **Never override base-game on_actions.** Always subscribe via the nesting pattern. Writing `on_law_enacted = { effect = { ... } }` at top level replaces vanilla's entire on_action handler -- every base-game event tied to law enactment stops firing.
   ```
   on_law_enacted = {
       on_actions = {
   ...
   ```
2. **Confirm the hook exists in vanilla.** Fetch `common/on_actions/00_on_actions.txt` from the mirror, or grep the existing mod (we already subscribe to `on_game_started`).
3. **Know the scope the hook gives you.** `on_law_enacted` -- country scope. `on_monthly_pulse_country` -- country scope. `on_character_death` -- character scope. Using the wrong scope reference inside the effect is a common silent failure.
