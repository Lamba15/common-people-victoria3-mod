# Victoria 3 Modding Reference (1.13 line, verified against vanilla)
...
Diplomatic-play scope:
- `on_diplo_play_war_start` (root = diplomatic play; enter `scope:actor` / `scope:target` for the lead countries)
- `on_war_end` (root = diplomatic play; `scope:actor` / `scope:target` are available)
...
When unsure about a hook name or scope, grep vanilla `game/common/on_actions/00_code_on_actions.txt`.

### Never override vanilla

Don't write `on_law_enactment_pass = { effect = { ... } }` at top level -- that replaces the vanilla one. Always nest via the `on_actions = { ... }` subscription pattern so every subscriber runs.
...
10. **`should_popup_when_expiring = no`** -- use for hidden-ish events that should silently time out with the default option rather than forcing a popup.
